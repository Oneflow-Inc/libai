...
class TokenBlockDataset(flow.utils.data.Dataset):
    """Break a Dataset of tokens into blocks.

    Args:
        dataset (~flow.utils.data.Dataset): dataset to break into blocks
    ...
    def __getitem__(self, index):
        ...
        if self.include_targets:
            ...
            if s == 0:
                source = flow.cat([item.new([self.eos]), buffer[0 : e - 1]])
                past_target = flow.cat(
                    [item.new([self.pad, self.eos]), buffer[0 : e - 2]]
                )
            else:
                source = buffer[s - 1 : e - 1]
                if s == 1:
                    past_target = flow.cat([item.new([self.eos]), buffer[0 : e - 2]])
                else:
