"""
Dataset class and data loader definition of Standford Cars dataset
"""
...
class StanfordCarsDataset(datasets.ImageFolder):
    def __init__(
        self, root: str, train: bool = True, transform: Optional[Callable] = None, **kwargs
    ):
        prefix = "train" if train else "test"
        root = osp.join(root, prefix)
        super(StanfordCarsDataset, self).__init__(root=root, transform=transform, **kwargs)
...
dataloader.test = [
    LazyCall(build_image_test_loader)(
        dataset=LazyCall(StanfordCarsDataset)(
            root="./dataset",
            train=False,
            transform=test_aug,
        ),
