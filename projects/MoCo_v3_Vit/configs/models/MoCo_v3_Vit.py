...
class VisionTransformerMoCo(VisionTransformer):
    ...
    def build_2d_sincos_position_embedding(self, temperature=10000.):
        ...
        grid_w = flow.arange(w, dtype=flow.float32).cuda().to_global(sbp=self.pos_embed.sbp, placement=self.pos_embed.placement)
        grid_h = flow.arange(h, dtype=flow.float32).cuda().to_global(sbp=self.pos_embed.sbp, placement=self.pos_embed.placement)
        grid_w, grid_h = flow.meshgrid(grid_w, grid_h)
        assert self.embed_dim % 4 == 0, 'Embed dimension must be divisible by 4 for 2D sin-cos position embedding'
        pos_dim = self.embed_dim // 4
        omega = (flow.arange(pos_dim, dtype=flow.float32) / pos_dim).cuda().to_global(sbp=self.pos_embed.sbp, placement=self.pos_embed.placement)
        omega = 1. / flow.tensor(temperature).cuda().to_global(sbp=self.pos_embed.sbp, placement=self.pos_embed.placement)**omega  
        ...
        pe_token = flow.zeros([1, 1, self.embed_dim], dtype=flow.float32).cuda().to_global(sbp=self.pos_embed.sbp, placement=self.pos_embed.placement)
