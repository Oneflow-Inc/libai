...
class RobertaPreTrainingHeads(nn.Module):
    def __init__(self, vocab_size, hidden_size, init_method):
        super().__init__()
        self.predictions = RobertaLMPredictionHead(hidden_size, init_method)
        self.lm_logits = LMLogits(vocab_size, bias=True)
        self.loss_func = RobertaLoss()
    ...
    def forward(
        ...
        if lm_labels is not None:
            return self.loss_func(prediction_scores, lm_labels, loss_mask)
        return {
...
class RobertaForPreTraining(nn.Module):
    """Roberta Model with two heads on top as done during the pretraining: a
    ...
    def __init__(self, cfg):
        ...
        self.cls_head = RobertaPreTrainingHeads(
            cfg.vocab_size,
            cfg.hidden_size,
    ...
    def forward(
        ...
        outputs = self.Roberta(input_ids, attention_mask, tokentype_ids)
        sequence_output, _ = outputs[:2]

        return self.cls_head(
            sequence_output,
            self.Roberta.word_embeddings_weight(),
            lm_labels,
            loss_mask,
        )
    ...
    def set_pipeline_stage_id(model):
        ...
        for module_block in model.modules():
            ...
            elif isinstance(module_block.origin, RobertaPreTrainingHeads):
                module_block.config.stage_id = dist_utils.get_layer_stage_id(-1)
