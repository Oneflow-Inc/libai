...
class GPTDataset(flow.utils.data.Dataset):
    ...
    def __init__(self, name, tokenizer, data_prefix, indexed_dataset,
                 max_num_samples, max_seq_length, seed=1234):
...
def _build_index_mappings(name, data_prefix, documents, sizes,
    ...
    if flow.env.get_rank() == 0:
        if (not os.path.isfile(doc_idx_filename)) or \
           ...
           (not os.path.isfile(shuffle_idx_filename)):
            ...
            logger.info(' > WARNING: could not find index map files, building '
            ...
            logger.info(' > elasped time to build and save doc-idx mapping '
            ...
            logger.info(' > elasped time to build and save shuffle-idx mapping'
                         ' (seconds): {:4f}'.format(time.time() - start_time))
    ...
    # Load mappings.
    start_time = time.time()
    logger.info(' > loading doc-idx mapping from {}'.format(
        doc_idx_filename))
    doc_idx = np.load(doc_idx_filename, allow_pickle=True, mmap_mode='r')
    logger.info(' > loading sample-idx mapping from {}'.format(
        sample_idx_filename))
    sample_idx = np.load(sample_idx_filename, allow_pickle=True, mmap_mode='r')
    logger.info(' > loading shuffle-idx mapping from {}'.format(
