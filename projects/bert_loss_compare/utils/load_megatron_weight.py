...
def convert_tensor(tensor: torch.Tensor):
    tensor = tensor.float()
    return flow.Tensor(tensor.cpu().numpy(), sbp=dist.get_nd_sbp([flow.sbp.broadcast, flow.sbp.broadcast]), placement=dist.get_layer_placement(0))
...
def load_tensor(tensor_lhs, tensor_rhs):
    tensor_rhs = tensor_rhs.to_global(placement=tensor_lhs.placement, sbp=tensor_lhs.sbp)
...
def load_model(model: flow.nn.Module, state_dict):
    ...
    for k in list(state_dict.keys()):
        if k in model_state_dict:
            # After align `libai.layer.Linear` with `torch.nn.Linear`,
