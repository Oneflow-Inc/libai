...
def train_valid_test_dataset_provider(cfg, train_val_test_num_samples):
    """Build train, valid, and test datasets."""

    logger.info("> building train, validation, and test datasets ")
    train_ds, valid_ds, test_ds = build_train_valid_test_datasets(
        data_prefix=cfg.data.data_path,
        ...
        seed=cfg.train.seed,
        skip_warmup=(not cfg.data.mmap_warmup),
        binary_head=True,
        dataset_type="bert_cn_wwm"
        if cfg.data.tokenizer_type == "BertCNWWMTokenizer"
        else cfg.data.dataset_type,
    )
    logger.info(f"> finished creating {cfg.data.dataset_type} datasets ...")
