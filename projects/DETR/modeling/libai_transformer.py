...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        encoder_layer = TransformerLayer(d_model, nhead, dim_feedforward,
                                        ...
                                        output_dropout_prob=dropout,
                                        drop_path_prob=dropout)
        
        encoder_norm = nn.LayerNorm(d_model) if normalize_before else None
        self.encoder = TransformerEncoder(encoder_layer, num_encoder_layers, nhead, encoder_norm)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src,
        ...
        for layer in self.layers:
            query, key = self.with_pos_embed(output, pos)
            src_len, _, _ = key.shape
            attn_mask = attn_mask_fusion(src_key_padding_mask, mask, bsz, src_len, self.nhead)
            output = layer(output, attention_mask=attn_mask)
...
def attn_mask_fusion(key_padding_mask, attention_mask, bsz, src_len, n_heads):
    
    # merge key padding and attention masks
    if key_padding_mask is not None:
        assert key_padding_mask.shape == (bsz, src_len), \
            f"expecting key_padding_mask shape of {(bsz, src_len)}, but got {key_padding_mask.shape}"            
        key_padding_mask = key_padding_mask.view(bsz, 1, 1, src_len).expand(-1, n_heads, -1, -1).reshape(bsz * n_heads, 1, src_len)
