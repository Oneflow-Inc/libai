...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        encoder_layer = TransformerLayer(d_model, dim_feedforward, nhead, dropout_prob=dropout)
        encoder_norm = nn.LayerNorm(d_model) if normalize_before else None
        self.encoder = TransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm)


        decoder_layer = TransformerLayer(d_model, dim_feedforward, nhead, dropout_prob=dropout, is_decoder=True)
...
#         new_attention_mask = flow.zeros_like(attention_mask).to(dtype=flow.float)
