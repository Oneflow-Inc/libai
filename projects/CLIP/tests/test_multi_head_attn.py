...
class TestMultiHeadAttention(unittest.TestCase):
    def test_with_torch(self):
        ...
        torch_out, _ = multi_head_attention_forward_torch(
            query=x_torch, key=x_torch, value=x_torch,
           ...
           in_proj_weight=None,
           in_proj_bias=torch.cat([torch.from_numpy(q_proj_bias), torch.from_numpy(k_proj_bias), torch.from_numpy(v_proj_bias)]),
           bias_k=None,
           bias_v=None,
           add_zero_attn=False,
           dropout_p=0,
        ...
        flow_out, _ = multi_head_attention_forward_flow(
            query=x_flow, key=x_flow, value=x_flow,
           ...
           in_proj_weight=None,
           in_proj_bias=flow.cat([flow.from_numpy(q_proj_bias).cuda(), flow.from_numpy(k_proj_bias).cuda(), flow.from_numpy(v_proj_bias).cuda()]),
           bias_k=None,
           bias_v=None,
           dropout_p=0,
