    ...
    for name, tensor in state.items():
        if 'embeddings' in name:
            ...
            if 'self' in name:
                ...
                qkv_w = torch.cat((state[q_w].permute(1,0), state[k_w].permute(1,0), state[v_w].permute(1,0)), dim=-1)
                ...
                if 'dense' in name:
                    if 'weight' in name:
                        ...
                        save[target] = convert_tensor(tensor.permute(1,0))
                    elif 'bias' in name:
            ...
            save[t_w] = convert_tensor(state[w].permute(1,0))
            ...
            if 'weight' in name:
                save['pooler.dense.weight'] = convert_tensor(tensor.permute(1,0))
            elif 'bias' in name:
...
def load_huggingface_bert(model, path, layers=12):
    huggingface_state_dict = torch.load(path)
    of_state_dict, _ = conver_state(huggingface_state_dict, layers=layers)
