...
def build_training_sample(sample, target_seq_length,
    ...
    train_sample = Instance(
        tokens_enc=DistTensorData(tensor=flow.tensor(tokens_enc), placement_idx=0),
        tokens_dec=DistTensorData(tensor=flow.tensor(tokens_dec_in), placement_idx=0),
        encoder_attn_mask=DistTensorData(tensor=flow.tensor(enc_mask), placement_idx=0),
        decoder_attn_mask=DistTensorData(tensor=flow.tensor(dec_mask), placement_idx=0),
        encoder_decoder_attn_mask=DistTensorData(tensor=flow.tensor(enc_dec_mask), placement_idx=0),
        labels=DistTensorData(tensor=flow.tensor(labels), placement_idx=0),
        loss_mask=DistTensorData(tensor=flow.tensor(loss_mask), placement_idx=0),
        # truncated=DistTensorData(tensor=flow.tensor(int(truncated)), placement_idx=0),
    )
    # train_sample = Instance(
    #     'text_enc'=DistTensorData(tensor=flow.tensor(tokens_np), placement_idx=0),
    #     'text_dec'=DistTensorData(tensor=flow.tensor(padding_mask_np), placement_idx=0),
...
def make_history_mask_3d(block):
    ...
    arange = flow.arange(length, device=block.device)
