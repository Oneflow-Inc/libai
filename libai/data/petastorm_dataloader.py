...
def _sanitize_oneflow_types(row_as_dict):
    """Promotes values types in a dictionary to the types supported by oneflow. Raises an error if type is clear error
    ...
    for name, value in row_as_dict.items():
        if isinstance(value, np.ndarray):
            if re.search('[SaUO]', value.dtype.str):
                raise TypeError('OneFlow does not support arrays of string or object classes. '
                                'Found in field {}.'.format(name))
        elif value is None:
            raise TypeError('OneFlow does not support nullable fields. Found None in {}'.format(name))
...
def decimal_friendly_collate(batch):
    ...
    We use ``decimal.Decimal`` types in petastorm dataset to represent timestamps. OneFlow's ``default_collate``
    ...
    :return: A dictionary of lists/oneflow.Tensor types
    """
...
class DataLoader(LoaderBase):
    ...
    def _iter_impl(self):
        ...
        for row in self.reader:
            ...
            # Promote some types that are incompatible with oneflow to be oneflow friendly.
            _sanitize_oneflow_types(row_as_dict)
...
class BatchedDataLoader(LoaderBase):
    """
    Same as DataLoader except it uses oneflow-based shuffling buffers which enable batched buffering
    ...
    def __init__(self, reader, batch_size=1,
        ...
        This class does not support special types that are not supported in OneFlow (decimal/string).

        :param reader: petastorm Reader instance
        :param batch_size: the number of items to return per batch; factored into the len() of this reader
        :param transform_fn: an optional callable to convert batches from the reader to OneFlow tensors
        :param shuffling_queue_capacity: Queue capacity is passed to the underlying :class:`tf.RandomShuffleQueue`
          instance. If set to 0, no shuffling will be done.
        """
        ...
        self.reader = reader
        self.batch_size = batch_size
        self.transform_fn = transform_fn or flow.as_tensor
...
def _load_rows_into_mem(reader, transform_fn, rows_capacity):
    ...
    :param reader: petastorm Reader instance.
    :param transform_fn: transform function which converts batches from the reader to OneFlow tensors
    ...
    for row in reader:
        ...
        if buffer is None:
            ...
            buffer = []
            for v in items:
                buffer.append(flow.empty((rows_capacity,) + v.shape[1:], dtype=v.dtype, device=v.device))
...
class InMemBatchedDataLoader(object):
    ...
    def __init__(self, reader, batch_size=1,
        ...
        self._keys, self._buffer = _load_rows_into_mem(reader, transform_fn or flow.as_tensor, rows_capacity)
    ...
    def __iter__(self):
        ...
        for epoch in range(self._num_epochs):
            ...
            if self._shuffle:
                ...
                indices = flow.randperm(size, generator=g).tolist()
            else:
