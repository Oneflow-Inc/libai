...
class BatchedShufflingBufferBase(object):
    ...
    def add_many(self, items):
        items = [flow.as_tensor(v) for v in items]
    ...
    def _add_many(self, items):
        ...
        if len(self._buffer) == 0:
            ...
            for i, v in enumerate(items):
                self._buffer[i] = flow.cat([self._buffer[i][self._batch_start_idx:], v], 0)
...
class BatchedRandomShufflingBuffer(BatchedShufflingBufferBase):
    ...
    def _add_many(self, items):
        ...
        if self._items is None:
            ...
            self._items = []
            for v in items:
                self._items.append(flow.empty((new_capacity,) + v.shape[1:], dtype=v.dtype, device=v.device))
        ...
        if new_capacity > self._items[0].shape[0]:
            for k, v in enumerate(self._items):
                self._items[k] = flow.empty((new_capacity,) + v.shape[1:], dtype=v.dtype, device=v.device)
    ...
    def retrieve(self):
        ...
        if self._random_indices is None:
            ...
            self.next_sample_head = 0
            self._random_indices = flow.randperm(int(self._size), device=self._items[0].device)
