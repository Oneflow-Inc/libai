...
from libai.layers.activation import build_activation, Activation
...
class WindowAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.p = proj_drop
        self.logit_scale = nn.Parameter(flow.log(10 * flow.ones(num_heads, 1, 1,
        ...
        self.cpb_mlp = nn.Sequential(Linear(2, 512, bias=True, layer_idx=layer_idx),
                                     nn.ReLU(inplace=True),
                                     Linear(512, num_heads, bias=False, layer_idx=layer_idx))

        # TODO: get relative_coords_table
        relative_coords_h = flow.arange(-(self.window_size[0] - 1), self.window_size[0], dtype=flow.float32)
        relative_coords_w = flow.arange(-(self.window_size[1] - 1), self.window_size[1], dtype=flow.float32)
        relative_coords_table = flow.stack(
            flow.meshgrid(*[relative_coords_h,
        ...
        relative_coords_table = flow.sign(relative_coords_table) * flow.log2(
            flow.abs(relative_coords_table) + 1.0) / flow.log2(flow.tensor([8], dtype=flow.float32))
        ...
        if qkv_bias:
            self.q_bias = nn.Parameter(flow.zeros(dim,
                                                  placement=dist.get_layer_placement(layer_idx),
                                                  sbp=dist.get_nd_sbp([flow.sbp.broadcast, flow.sbp.broadcast]), ))
            self.v_bias = nn.Parameter(flow.zeros(dim,
                                                  placement=dist.get_layer_placement(layer_idx),
                                                  sbp=dist.get_nd_sbp([flow.sbp.broadcast, flow.sbp.broadcast]), ))
        else:
            self.q_bias = None
    ...
    def forward(self, x, mask=None):
        ...
        if self.q_bias is not None:
            qkv_bias = flow.concat((self.q_bias,
                                    flow.zeros(self.v_bias.shape, requires_grad=False,
        ...
        logit_scale = flow.clamp(self.logit_scale, min=-1e6, max=math.log(1. / 0.01)).exp()
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = WindowAttention(
            ...
            fused_bias_add_dropout=True, layer_idx=layer_idx)
        ...
        self.mlp = MLP(hidden_size=dim, ffn_hidden_size=mlp_hidden_dim, output_dropout_prob=drop,
                       bias_gelu_fusion=True, bias_dropout_fusion=True, layer_idx=layer_idx)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self,
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size).to_global(
            placement=dist.get_layer_placement(layer_idx),
            sbp=dist.get_nd_sbp([flow.sbp.broadcast, flow.sbp.broadcast]),
        )
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_embed = PatchEmbed(
            ...
            norm_layer=norm_layer if self.patch_norm else None, layer_idx=0)
