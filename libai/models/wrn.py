...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv2(out)
        return flow.add(x if self.equalInOut else self.convShortcut(x), out)
...
class WideResNet(nn.Module):
    @configurable
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0, loss_func=None):
    ...
    def forward_features(self, x, is_feat=False, preact=False):
        out = self.conv1(x)
        f0 = out
        out = self.block1(out)
        f1 = out
