...
class timmAttention(nn.Module):
    """Multi-head attention layer modified from pytorch-image-models for vision tasks 
    which supports both self attention and cross attention.
    ...
    def forward(
        ...
        if self.is_cross_attention:
            ...
            # split query, key, value like timm
            query_key_value = self.query_key_value(hidden_states)
            query_key_value = query_key_value.reshape(bsz, tgt_len, 3, self.num_heads, self.head_size)
