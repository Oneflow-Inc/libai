...
from libai.models.vision_transformer import VisionTransformerGraph
from configs.common.models.vit.vit_tiny_patch16_224 import model as vit_tiny_patch16_224


class TestViTGraph(TestCase):
    def test_vit_tiny_patch16_224(self):
        random_input = flow.randn(1, 3, 224, 224).to_consistent(sbp=dist.get_nd_sbp([flow.sbp.split(0), flow.sbp.broadcast]),
    ...
    def test_vit_tiny_patch16_224_graph(self):
        random_input = flow.randn(1, 3, 224, 224).to_consistent(sbp=dist.get_nd_sbp([flow.sbp.split(0), flow.sbp.broadcast]),
                                                                placement=flow.placement("cuda" if flow.cuda.is_available() else "cpu", {0:0}))
        targets = flow.zeros(1, dtype=flow.int64).to_consistent(sbp=dist.get_nd_sbp([flow.sbp.split(0), flow.sbp.broadcast]), 
                                                                placement=flow.placement("cuda" if flow.cuda.is_available() else "cpu", {0:0}))
        model = instantiate(vit_tiny_patch16_224)
        model.apply(dist.convert_to_distributed_default_setting)
