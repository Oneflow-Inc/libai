...
def get_sample(mode: str):
    ...
    data_path = Path("/workspace/data/samples")
    tokens_enc = np.load(data_path / "tokens_enc.npy")[0:16]
    tokens_dec = np.load(data_path / "tokens_dec.npy")[0:16]
    enc_mask = np.load(data_path / "encoder_attn_mask.npy")[0:16]
    dec_mask = np.load(data_path / "decoder_attn_mask.npy")[0:16]
    enc_dec_mask = np.load(data_path / "encoder_decoder_attn_mask.npy")[0:16]
    func = {
        "flow": numpy_to_flow,
...
def get_random_sample(vocab_size):
    ...
    flow_tensors = tuple(
        map(numpy_to_flow, [tokens_enc, tokens_dec, ~enc_mask, ~dec_mask, ~enc_dec_mask])
    )
    torch_tensors = tuple(
        map(
...
def load_megatron_embedding_weight(flow_model: flow.nn.Module, torch_model: torch.nn.Module):
    ...
    if "tokentype_embeddings.weight" in torch_state and "tokentype_embeddings.weight" in flow_state:
        ...
        convert_and_copy_tensor(
            flow_state["tokentype_embeddings.weight"], tokentype_embeddings_weight
        )
