...
if __name__ == "__main__":
    ...
    from utils import get_random_sample, get_sample, load_megatron_weight
    ...
    megatron_t5.language_model.encoder.layers[0].self_attention.dense.register_forward_hook(
        mega_hook
    )
    ...
    print(
        "max diff: ", np.max(np.max(flow_output.numpy() - megatron_output.detach().cpu().numpy()))
    )
