...
export DEBUG_FILE="/HOME/scw6cab/run/OCCL/oneflow/log/oneflow_cpu_rank_"
...
rm -rf /HOME/scw6cab/run/OCCL/libai/log
mkdir -p /HOME/scw6cab/run/OCCL/libai/log

rm -rf /HOME/scw6cab/run/OCCL/oneflow/log
mkdir -p /HOME/scw6cab/run/OCCL/oneflow/log
...
if [ "$RUN_TYPE" == "PURE" ];then
    ...
    if [ ! -d "/HOME/scw6cab/run/OCCL/oneflow/log/nsys" ];then
        mkdir -p /HOME/scw6cab/run/OCCL/oneflow/log/nsys
    fi
    # cmd="nsys profile -f true --trace=cuda,cudnn,cublas,osrt,nvtx -o /HOME/scw6cab/run/OCCL/oneflow/log/nsys/$NSYS_FILE python3 -m oneflow.distributed.launch"
    cmd="nsys profile -f true -o /HOME/scw6cab/run/OCCL/oneflow/log/nsys/$NSYS_FILE python3 -m oneflow.distributed.launch"
fi
echo cmd=$cmd
...
$cmd \
  ...
  > /HOME/scw6cab/run/OCCL/oneflow/log/oneflow.log 2>&1
